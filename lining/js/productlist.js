;(function(){
    ...
    // 获取本地存储的数据，确定请求的是哪一部分的数据
    // $classification = localStorage.getItem("classification");
    $(function(){
        // $.ajaxSettings.async = false; // 异步变同步
        $.get("http://47.115.92.110/lining/php/select_li.php", {classification: classification}, function({error, data}){
            if(!error){
                // 获取所有数据
                alldata = data;
                // 截取数据
        ...
        for (let i = 0; i < arr.length; i++){
            ...
            let str = `
                <li class="list-group-item">
                    <a href="http://47.115.92.110/lining/html/details.html?classification=${classification}&sku=${item.sku}">
